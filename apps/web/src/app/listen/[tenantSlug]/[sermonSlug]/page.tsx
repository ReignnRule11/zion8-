...
export default async function PublicSermonPage({
  ...
  params: Promise<{ tenantSlug: string; sermonSlug: string }>;
}) {
  const { tenantSlug, sermonSlug } = await params;
  let sermon;
  try {
    sermon = await api.getPublicSermon(tenantSlug, sermonSlug);
  } catch (error) {
  ...
  return (
    <main className="mx-auto min-h-screen w-full max-w-3xl space-y-6 px-6 py-12">
      <p className="text-sm text-slate-400">Zion8 public sermon</p>
      <header>
